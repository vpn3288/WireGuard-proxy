...
# Pick the first with highest port (or just first)
r = results[0]
print(f"FOUND")
print(f"PORT={r['port']}")
...
print(f"PATH={r['path']}")
print(f"UUIDS={'|'.join(r['uuids'])}")
print(f"SHORTIDS={'|'.join(r['shortids'])}")
e = r.get('extra', {})
print(f"XHTTP_PATH={e.get('xhttp_path','/')}")
print(f"XHTTP_HOST={e.get('xhttp_host','')}")
print(f"XHTTP_MODE={e.get('xhttp_mode','auto')}")
print(f"WS_PATH={e.get('ws_path','/')}")
print(f"WS_HOST={e.get('ws_host','')}")
print(f"GRPC_SERVICE={e.get('grpc_service','')}")
PYEOF
    ) || return 1

    echo "$sniff_result" | grep -q "^FOUND" || return 1

    _kv() { echo "$sniff_result" | grep -m1 "^${1}=" | cut -d= -f2-; }
...
for ib in inbounds:
    ...
    print(f"SHORTIDS={'|'.join(short_ids) if short_ids else ''}")
    print(f"XHTTP_PATH={extra.get('xhttp_path','/')}")
    print(f"XHTTP_HOST={extra.get('xhttp_host','')}")
    print(f"XHTTP_MODE={extra.get('xhttp_mode','auto')}")
    print(f"WS_PATH={extra.get('ws_path','/')}")
    print(f"WS_HOST={extra.get('ws_host','')}")
    print(f"GRPC_SERVICE={extra.get('grpc_service','')}")
    sys.exit(0)
...
for row in rows:
    ...
    print(f"UUIDS={'|'.join(uuids)}")
    print(f"SHORTIDS={'|'.join(shortids_clean)}")
    print(f"XHTTP_PATH={extra.get('xhttp_path','/')}")
    print(f"XHTTP_HOST={extra.get('xhttp_host','')}")
    print(f"XHTTP_MODE={extra.get('xhttp_mode','auto')}")
    print(f"WS_PATH={extra.get('ws_path','/')}")
