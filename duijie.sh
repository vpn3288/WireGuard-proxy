#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════╗
# ║  duijie.sh  v4.2  —  CN2GIA中转 ↔ 落地机  WireGuard对接            ║
...
# ╚══════════════════════════════════════════════════════════════════════╝
#
# ── v4.2 修复 & 新增清单（第三轮代码审查：1项真实BUG + 4项误报澄清）─────────────
#
#  继承 v4.1 全部修复，本版修复第三轮代码审查的唯一真实 BUG：
#
#  [FIX-X 高] _strip_jsonc() — 正则表达式无法识别字符串边界（真实 BUG）
#       旧版：(?<!:)(?<!https)//[^
]*
#       缺陷：负向后视 (?<!:) 只看 1 个字符，无法区分 JSON 键名中的 //
#       失败案例：{"//key": "value"}  → 正则误删 "//key" → "Unterminated string"
#             {"url": "http://x.com"} → 正则误删 → 解析失败
#       → 改为状态机实现：逐字符维护 in_string 标志，字符串内原样传递，
#         字符串外才删除 // 行注释和 /* */ 块注释。
#         使用 BS=chr(92) 规避 << PYEOF heredoc 展开时 \ 转义歧义。
#         覆盖所有 JSONC 变体：键名含//、URL含//、字符串含//、转义引号内含//。
#
#  ── 第三轮审查误报澄清（以下4项均已在 v4.1 或更早版本中正确处理）────────
#
#  [已实现] PostUp/PreDown 策略路由对称清理（v2.8 PPYEOF 块）
#       审查报告描述的 PostDown 缺失问题：脚本中 _wg::setup_local() 的 Python
#       PPYEOF 块在写入 wg0.conf 时，postup 和 predown 均已包含：
#         PostUp  = ip route replace default dev wg0 table {table};
#                   ip rule add from {wg_ip} table {table} priority 100
#         PreDown = ip rule del from {wg_ip} table {table} 2>/dev/null || true;
#                   ip route del default dev wg0 table {table} 2>/dev/null || true
#       完全对称，systemctl restart wg-quick@wg0 不会产生规则堆积。
#
#  [已实现] WG_MTU 空字符串保底（main() line 2762）
#       _wg::detect_mtu() 函数所有退出路径均设置 WG_MTU；
#       main() 额外追加 WG_MTU="${WG_MTU:-1380}" 作为双重保底；
#       detect_mtu 远端 SSH 命令均有 || echo "$fallback" 兜底。
#
#  [已实现] 中转机 iptables 规则持久化（_fw::dnat_add 远端 heredoc）
#       检测并安装 iptables-persistent/netfilter-persistent，
#       或写 /etc/iptables/rules.v4 + systemd restore 服务三重方案，
#       中转机重启后规则自动恢复。
#
#  [已实现] 落地机端口隔离（_wg::setup_local DROP 兜底规则 v3.0）
#       iptables -A INPUT ! -i wg0 -p tcp --dport ${LUODI_PORT} -j DROP
#       （阻断非 wg0 来源的代理端口访问），配合 netfilter-persistent 持久化。
#       此类规则通过 iptables-persistent 持久化而非 wg0.conf PostUp/PostDown，
#       避免了与 wg-quick 钩子系统的双重管理混乱。
#
#  [已实现] 中转机侧 nc/bash-tcp 双路验证（_check::connectivity v3.0）
#       路径1：中转机 127.0.0.1:RELAY_PORT → DNAT → WG → 落地机 Xray
#       路径2：中转机 → LUODI_WG_ASSIGNED_IP:LUODI_PORT（WG 内网直连）
#       优先使用 nc -z -w5，降级到 bash /dev/tcp。
#
# ── v4.1 修复 & 新增清单（第二轮代码审查：5项真实BUG修复）────────────
#
#  继承 v4.0 全部修复，本版修复代码审查文档中确认的5项真实问题：
#
#  [FIX-A 严重] _out::save() 远端 nodes.json — 并发写入竞态
#       旧版 open(nodes_path,"w") 直接覆盖，与 peer_map 旧问题完全相同。
#       两个 duijie.sh 并发时：A/B同时读取 → A写入 → B覆盖A → A数据丢失。
#       → 与 FIX-1 策略一致：fcntl.flock(LOCK_EX) + os.replace() 原子写入
#         + 5次重试 + fsync + chmod 0o600。
#
#  [FIX-B 高] _fw::dnat_add() 远端 heredoc — sysctl.conf 写入不一致残留
#       FIX-6 已在 _info::read_relay() 修正为写 sysctl.d/99-duijie.conf，
#       但 _fw::dnat_add 内的远端 SHEOF 仍有一段直接 sed /etc/sysctl.conf。
#       两处并存导致维护混乱，升级后可能残留旧配置。
#       → 统一改为 mkdir /etc/sysctl.d + echo > 99-duijie.conf + sysctl --system
#         与 read_relay() 保持完全一致，全局只有一个写入点。
#
#  [FIX-C 高] set -euo pipefail + grep 管道 — 静默退出风险
#       grep 找不到匹配时返回码 1，在 set -e 下会导致脚本直接退出。
#       受影响的典型场景：_del_rules_by_comment 管道内的 grep，
#       以及其他 iptables -S | grep ... 链式命令。
#       → 在所有可能无匹配的 grep 操作末尾补充 || true；
#         _del_rules_by_comment 内层 while 管道加保护；
#         远端 heredoc 的 set -e 片段同步检查。
#
#  [FIX-D 中] _wg::setup_local() — mktemp fallback 路径可预测
#       旧版 mktemp || /tmp/.wg_key_$$ 中 $$ 为 PID，可被同机器其他用户
#       预测并预创建符号链接，私钥写入时发生 symlink attack。
#       → 改为 mktemp -p /tmp duijie.XXXXXX（随机后缀，安全）；
#         若 mktemp 仍然失败则使用 /dev/shm（内存文件系统，不落盘）降级。
#
#  [FIX-E 低] {{1}} 误报澄清（非BUG，保留现有代码）
#       审查文档将 used = {{1}} 标为 Python 语法错误。
#       实际：该代码位于本地 Python f-string 内（remote = f"""..."""），
#       {{1}} 在 f-string 中是对 {1} 的转义，生成的远端脚本中正是 used = {1}
#       （Python set 字面量），运行时完全正确。无需修改，此处加注释说明。
#
#  ── v4.0 修复 & 新增清单（代码审查：6项安全/健壮性真实修复）────────────
#
#  继承 v3.0 全部修复，本版基于完整代码审查实施6项真实改动：
#
#  [FIX-1 严重] _wg::allocate_ip() — peer_map 并发写入竞态修复
#       旧版 save_peer_map() 使用 open(...,"w") 直接覆盖，多落地机并发运行
#       duijie.sh 时存在竞态条件：两进程同时读到相同空闲IP → IP冲突。
#       → 使用 fcntl.flock(LOCK_EX) + os.replace() 原子写入：
#         · LOCK_EX 阻塞直到上一进程释放锁，串行化写入窗口
#         · os.replace() 原子替换 tmp→final，读进程不会读到半写文件
#         · 5次重试保障（0.1s间隔），fsync 确保落盘
#         · os.chmod(0o600) 收紧权限（防止其他用户读取私钥映射）
#
#  [FIX-2 高] _wg::add_peer() — SSH heredoc 变量展开安全加固
#       旧版 << SHEOF（无引号）在本地 bash 展开变量后发往中转机。
#       WG 公钥/UUID/IP 均为安全字符集，无实际注入风险，
#       但规范性不足；内嵌 Python << PYEOF 同理。
#       → 外层改为 << 'SHEOF'（单引号，禁止本地展开），
#         通过 bash -s -- arg1 arg2 ... 传参，远端赋值 LUODI_PUBKEY="$1"；
#         内嵌 Python << PYEOF（无引号）继续展开已赋值的 bash 变量，
#         保持与旧版功能完全一致，同时消除理论上的特殊字符风险。
#         · _ssh::pipe_args() 新增辅助函数（支持 key/password 双认证）
#
#  [FIX-3 高] _fw::dnat_add() — nftables 后端检测与兼容性警告
#       Debian 11+/Ubuntu 22+ 默认使用 nftables，iptables 命令
#       实为 iptables-nft（映射层），通常兼容但持久化行为有差异。
#       → 在写入规则前检测后端类型（iptables-nft vs iptables-legacy），
#         iptables-nft 时输出兼容性提示（不阻断，继续执行）；
#         提示中包含建议操作供用户参考。
#
#  [FIX-4 中] _wg::detect_mtu() — ping -M do 参数兼容性
#       Alpine / 部分 OpenWRT / 老旧内核不支持 ping -M do 设置 DF 标志，
#       导致 DF-ping 阶梯探测静默失败，全部回退到保底值 1380。
#       → 探测前先在中转机测试 ping -M do 是否可用：
#         · 可用：沿用原有 DF-ping 阶梯探测逻辑
#         · 不可用：输出明确提示（原因+推荐手动值），直接使用保底 1380
#         · 避免静默回退掩盖真实的 -M do 不支持问题
#
#  [FIX-5 中] _ssh::setup() + _ssh::run/pipe — SSH 命令超时防卡死
#       旧版 ConnectTimeout=10 仅保护建连阶段，远端长耗时命令（Python
#       IP 分配、iptables save）卡死时本地 ssh 会无限等待。
#       → SSH_OPTS 追加 -o ServerAliveInterval=15 -o ServerAliveCountMax=4
#         （共 60s 无响应后 SSH 自动断开）；
#         远端关键 Python 脚本包裹 timeout 60，防止单点卡死。
#
#  [FIX-6 低] _info::read_relay() — sysctl ip_forward 写入 sysctl.d
#       旧版直接 sed /etc/sysctl.conf，不符合现代 Linux 配置规范，
#       且与发行版自身配置混杂，升级时易被覆盖。
#       → 改为写入 /etc/sysctl.d/99-duijie.conf（独立文件，易回滚），
#         使用 sysctl --system 重载（兼容 sysctl -p 的旧系统降级）；
#         文件不存在时创建，已存在时幂等更新。
#
#  [CORE] 以下 v3.0 条目核查无需变更（沿用）：
#       · peer_map LINK_ID 幂等复用       ── v2.8 已实现
#       · JSONC 注释剥离 _strip_jsonc()   ── v2.10 已实现
#       · 双路 TCP 握手 + TLS 深度验证    ── v3.0 已实现
#       · SEC 落地机 DROP 兜底规则        ── v3.0 已实现
#       · PostUp/PreDown 策略路由对称清理 ── v2.8 已实现
...
_ssh::setup() {
    ...
    # [FIX-5] ServerAliveInterval+CountMax：远端命令卡死时 60s 后自动断开（15s×4次）
...
_ssh::pipe_py() {
    case "$AUTH_TYPE" in
        # [FIX-5] timeout 60：防止远端 python3 执行卡死（IP分配/节点写入等操作）
        key)      ssh      -q "${SSH_OPTS[@]}"                          "${RELAY_SSH_USER}@${RELAY_IP}" "timeout 60 python3" ;;
        password) sshpass -p "$RELAY_SSH_PASS" ssh -q "${SSH_OPTS[@]}" "${RELAY_SSH_USER}@${RELAY_IP}" "timeout 60 python3" ;;
    esac
}

# [FIX-2] _ssh::pipe_args — 通过 bash -s -- args 传参，配合单引号 heredoc
#   避免本地 bash 展开 heredoc 内容，彻底消除特殊字符注入风险。
#   用法: _ssh::pipe_args "arg1" "arg2" ... << 'REMOTE_EOF'
#         远端脚本中用 $1 $2 ... 引用传入参数
#   Note: stdin heredoc 必须使用单引号 << 'REMOTE_EOF' （禁止本地展开）
_ssh::pipe_args() {
    # "$@" 在 -- 后传给远端 bash -s，远端脚本从 $1 $2 ... 读取
    case "$AUTH_TYPE" in
        # [FIX-5] bash -s 超时保护（同 pipe_py 一致，远端脚本最多执行 60s）
        key)      ssh      -q "${SSH_OPTS[@]}"                          "${RELAY_SSH_USER}@${RELAY_IP}" "bash -s" -- "$@" ;;
        password) sshpass -p "$RELAY_SSH_PASS" ssh -q "${SSH_OPTS[@]}" "${RELAY_SSH_USER}@${RELAY_IP}" "bash -s" -- "$@" ;;
    esac
}
...
def _strip_jsonc(text):
    # [FIX v4.2] 状态机实现 — 正确处理字符串上下文，覆盖一切 JSONC 注释场景
    #
    # 旧版缺陷（已修复）：
    #   正则 (?<!:)(?<!https)//[^\n]* 无法识别 JSON 字符串边界，
    #   导致 {"//key": "value"} 这类合法 JSON 键名被截断，
    #   json.loads() 抛出 "Unterminated string" 异常，配置文件修改失败。
    #
    # 新版策略：逐字符遍历，维护 in_string 状态；
    #   · 字符串外：遇 // 跳行尾，遇 /* 跳 */，其余直传
    #   · 字符串内：原样传递（含 "//key" 等合法内容），处理 \" \\ 转义
    #   · BS=chr(92) 规避 heredoc 展开时 \\ 歧义
    BS = chr(92)  # backslash，通过 chr() 规避 << PYEOF heredoc 转义歧义
    out = []; i = 0; n = len(text); in_s = False
    while i < n:
        c = text[i]
        if in_s:
            if c == BS and i + 1 < n:  # 转义序列：\" 或 \\ 等，跳过下一字符
            ...
            elif text[i:i+2] == '//':  # 行注释：跳到行尾（保留换行符本身）
                while i < n and text[i] != '\n':
                    i += 1
            elif text[i:i+2] == '/*':  # 块注释：跳到 */
                i += 2
...
_info::read_relay() {
    ...
    # [FIX-6] 确认中转机内核 IP 转发已开启（iptables DNAT 的前提条件）
    # 旧版直接写 /etc/sysctl.conf，与发行版配置混杂；
    # 改为写 /etc/sysctl.d/99-duijie.conf（独立文件，升级不被覆盖，易回滚）
    _ssh::run "mkdir -p /etc/sysctl.d && \
        echo 'net.ipv4.ip_forward=1' > /etc/sysctl.d/99-duijie.conf && \
        chmod 644 /etc/sysctl.d/99-duijie.conf && \
        ( sysctl --system 2>/dev/null || sysctl -q -p /etc/sysctl.d/99-duijie.conf 2>/dev/null || sysctl -q -w net.ipv4.ip_forward=1 )" 2>/dev/null || \
...
_wg::detect_mtu() {
    ...
    # [FIX-4] 先检测中转机 ping 是否支持 -M do（Alpine/OpenWRT/老内核不支持）
    local mdo_support
    mdo_support=$(_ssh::run "ping -c1 -W1 -M do -s 1 127.0.0.1 >/dev/null 2>&1 && echo yes || echo no" 2>/dev/null || echo "no")

    if [[ "$mdo_support" == "no" ]]; then
        WG_MTU="$fallback"
        echo ""
        echo -e "  ${YELLOW}╔══════════════════════════════════════════════════════════════════╗${NC}"
        echo -e "  ${YELLOW}║  ⚠  [FIX-4] ping -M do 不受中转机支持（Alpine/老内核/受限环境）  ║${NC}"
        echo -e "  ${YELLOW}║  无法执行 DF-ping MTU 探测，已使用保底值 WG_MTU=${WG_MTU}          ║${NC}"
        echo -e "  ${YELLOW}║  若遇断流/掉速，手动指定 MTU：WG_MTU=1360 bash duijie.sh          ║${NC}"
        echo -e "  ${YELLOW}╚══════════════════════════════════════════════════════════════════╝${NC}"
        echo ""
        return 0
    fi
...
def save_peer_map(pm):
    # [FIX-1] fcntl.flock(LOCK_EX) + os.replace() 原子写入，消除并发竞态
    import fcntl, time as _time
    dirp = os.path.dirname(peer_map_path)
    os.makedirs(dirp, exist_ok=True)
    tmp = peer_map_path + ".tmp"
    for _attempt in range(5):
        try:
            with open(tmp, "w") as f:
                fcntl.flock(f, fcntl.LOCK_EX)   # 阻塞直到获得排他锁
                json.dump(pm, f, indent=2)
                f.flush()
                os.fsync(f.fileno())              # 确保数据落盘
                fcntl.flock(f, fcntl.LOCK_UN)
            os.replace(tmp, peer_map_path)        # 原子替换，读进程不会读到半写文件
            os.chmod(peer_map_path, 0o600)        # 收紧权限，防止其他用户读取
            return
        except Exception as e:
            _time.sleep(0.1)
    print("[!] peer_map 写入失败（5次重试均未成功）", file=sys.stderr)
...
# ── 无历史记录，重新分配 ──────────────────────────────────────
used = {{1}}  # [FIX-E 澄清] 此处位于本地 Python f-string 内，{{1}} → {1}（set字面量），运行时语法正确
...
_wg::add_peer() {
    ...
    # [FIX-2] 改用单引号 heredoc << 'SHEOF'（禁止本地展开）+ pipe_args 传参
    # 参数顺序：$1=wg_pubkey $2=wg_ip $3=link_id $4=WG_MTU
    # 内嵌 Python << PYEOF（无引号）展开远端已赋值的 bash 变量，行为不变
    # [C3] 持久化通过直接写 wg0.conf 实现，不调用 wg-quick save
    _ssh::pipe_args "$wg_pubkey" "$wg_ip" "$link_id" "$WG_MTU" << 'SHEOF'
set -e
LUODI_PUBKEY="$1"
LUODI_WG_IP="$2"
LINK_ID="$3"
...
wg_mtu  = "$WG_MTU"    # [FIX-2] 引用远端 bash 变量（由 $4 赋值给 WG_MTU）
try:
...
# [FIX-2] 引用远端 bash 变量（由 $1/$2/$3 赋值，已在 set -e 后赋给 LINK_ID/LUODI_WG_IP/LUODI_PUBKEY）
python3 - << PYEOF
conf      = "/etc/wireguard/wg0.conf"
link_id   = "$LINK_ID"
wg_ip     = "$LUODI_WG_IP"
wg_pubkey = "$LUODI_PUBKEY"
...
PYEOF
    ...
    if ip link show wg0 &>/dev/null; then
        ...
        # [FIX-D] 安全临时文件：mktemp -p /tmp 随机后缀（防 PID 预测 symlink attack）
        # 旧版 /tmp/.wg_key_$$ 中 $$ 为可预测 PID，有 symlink 攻击面
        # 降级：/dev/shm（内存文件系统，不落盘，对安全性更好）
        local _wg_key_tmp
        _wg_key_tmp=$(mktemp -p /tmp duijie.XXXXXX 2>/dev/null)             || _wg_key_tmp=$(mktemp -p /dev/shm duijie.XXXXXX 2>/dev/null)             || _wg_key_tmp=$(mktemp 2>/dev/null)             || { log_error "mktemp 完全失败，无法安全存储WG私钥"; }
...
_fw::dnat_add() {
    ...
    # [FIX-3] 检测中转机 iptables 后端类型（nft vs legacy）
    # Debian 11+/Ubuntu 22+ 默认 iptables-nft，规则写入正常但持久化工具行为有差异
    local _nft_check
    _nft_check=$(_ssh::run "iptables --version 2>/dev/null | grep -o 'nf_tables\|legacy' || echo unknown" 2>/dev/null || echo "unknown")
    if [[ "$_nft_check" == "nf_tables" ]]; then
        echo ""
        echo -e "  ${YELLOW}╔══════════════════════════════════════════════════════════════════╗${NC}"
        echo -e "  ${YELLOW}║  ⚠  [FIX-3] 检测到中转机使用 iptables-nft 后端（非 legacy）     ║${NC}"
        echo -e "  ${YELLOW}║  规则写入通常兼容，但持久化工具行为可能有差异。                    ║${NC}"
        echo -e "  ${YELLOW}║  建议：确认 iptables-persistent 版本 ≥ 1.0.15 以支持 nft 后端；  ║${NC}"
        echo -e "  ${YELLOW}║  或使用 nft list ruleset 检查规则实际生效情况。                    ║${NC}"
        echo -e "  ${YELLOW}╚══════════════════════════════════════════════════════════════════╝${NC}"
        echo ""
    fi
...
# [FIX-B] 开启内核 IP 转发 — 统一写入 /etc/sysctl.d/99-duijie.conf
# 旧版 sed /etc/sysctl.conf 与 _info::read_relay() 的 sysctl.d 写法不一致
# 现在全局统一：独立文件，升级不覆盖，rm 即可回滚
mkdir -p /etc/sysctl.d
echo 'net.ipv4.ip_forward=1' > /etc/sysctl.d/99-duijie.conf
chmod 644 /etc/sysctl.d/99-duijie.conf
sysctl --system 2>/dev/null || sysctl -q -p /etc/sysctl.d/99-duijie.conf 2>/dev/null || sysctl -q -w net.ipv4.ip_forward=1
...
remote = f"""
import json, os, fcntl, time as _time

nodes_path = {json.dumps(nodes_path)}
link_id    = {json.dumps(link_id)}
entry      = json.loads({json.dumps(entry_json)})

# [FIX-A] nodes.json 原子写入（与 peer_map 策略相同）
# flock(LOCK_EX) + os.replace()，彻底消除并发竞态
def _load_nodes():
    try:
        return json.load(open(nodes_path))
    ...
    for _attempt in range(5):
        try:
            with open(tmp, "w") as _f:
                ...
                json.dump(nd, _f, indent=2, ensure_ascii=False)
            ...
            _time.sleep(0.1)
    print("[!] nodes.json 写入失败（5次重试均未成功）", file=__import__("sys").stderr)
...
# [FIX-A] 清理 nodes.json — 原子写入（flock + os.replace）
import fcntl, time as _time2
def _del_from_nodes():
    try:
        nd = json.load(open(nodes_path))
    except (FileNotFoundError, json.JSONDecodeError):
...
if _del_from_nodes():
    ...
    print(f"[!] 清理 nodes.json 失败（5次重试）")

# [IDEM v2.8] 同步清理 peer_map.json（释放 WG IP 供重用，flock 原子写入）
try:
    pm = json.load(open(peer_map_path))
    if del_lid in pm:
        freed_ip = pm.pop(del_lid)
        tmp_pm = peer_map_path + ".tmp"
        with open(tmp_pm, "w") as _f:
            fcntl.flock(_f, fcntl.LOCK_EX)
            json.dump(pm, _f, indent=2)
            _f.flush(); os.fsync(_f.fileno())
            fcntl.flock(_f, fcntl.LOCK_UN)
        os.replace(tmp_pm, peer_map_path)
...
_manage_mode() {
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║        节点管理  duijie.sh v4.2  WireGuard版        ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""
    _ssh::setup
    _info::read_relay
...
main() {
    clear
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║  duijie.sh  v4.0  —  WireGuard 隧道对接（单次解密）        ║${NC}"
