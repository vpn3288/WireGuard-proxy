...
manage_wg_keys() {
    ...
    if [[ "$force_new" == "false" && -f "$WG_KEY_FILE" && -s "$WG_KEY_FILE" ]]; then
        ...
        local yn
        read -rp "  复用已有密钥？（推荐 Y，已有 WG Peer 仍可用）[Y/n]: " yn </dev/tty || true
        if [[ "${yn,,}" != "n" ]]; then
            log_info "复用已有 WG 密钥"
            return 0
        fi
...
_show_sniff_result() {
    ...
    echo -e "  Reality公钥: ${LUODI_PUBKEY:0:20}${LUODI_PUBKEY:+...}${LUODI_PUBKEY:-（未获取）}"
    echo -e "  SNI       : ${LUODI_SNI:-（未获取）}"
...
def _emit(r, idx=None):
    ...
    print(f"{prefix}SECURITY={r.get('security','reality')}")
    print(f"{prefix}FLOW={r.get('flow','')}")
...
PYEOF
    ...
    # TLS/XTLS 选中时，公钥和 SNI 需要手动输入
    if [[ "${LUODI_SECURITY}" != "reality" ]]; then
        echo ""
        log_warn "TLS/XTLS inbound 无法自动获取 Reality 公钥，请手动输入："
        local i
        while [[ -z "$LUODI_PUBKEY" ]]; do
            read -rp "  Reality 公钥（base64，必填）: " i </dev/tty || true
            LUODI_PUBKEY="$i"
        done
        if [[ -z "$LUODI_SNI" ]]; then
            read -rp "  SNI（如 www.example.com，必填）: " i </dev/tty || true
            LUODI_SNI="$i"
        fi
        read -rp "  Short ID（可留空）: " i </dev/tty || true
        [[ -n "$i" ]] && shortids_raw="$i"
    fi
...
PYEOF
    ...
    if [[ -z "$LUODI_PUBKEY" ]]; then
        ...
        local i
        read -rp "Reality 公钥（base64）: " i </dev/tty || true
        LUODI_PUBKEY="$i"
...
fallback_manual_input() {
    ...
    _ask_field() {
        ...
        read -rp "  ${label} [${default:-必填}]: " i </dev/tty || true
        echo "${i:-$default}"
    }
...
_select_uuid() {
    ...
    local choice
    read -rp "  选择 [1]: " choice </dev/tty || true
    choice="${choice:-1}"
...
confirm_params() {
    ...
    local yn
    read -rp "  参数正确？[Y/n]: " yn </dev/tty || true
    if [[ "${yn,,}" == "n" ]]; then
        echo ""
        ...
        _fix() {
            local label="$1" var="$2"
            read -rp "  ${label} [${!var}]: " i </dev/tty || true
    ...
    if [[ -z "$LUODI_PORT"   ]]; then log_error "代理端口不能为空"; fi
    if [[ -z "$LUODI_UUID"   ]]; then log_error "UUID 不能为空"; fi
    if [[ "${LUODI_SECURITY:-reality}" == "reality" ]]; then
        if [[ -z "$LUODI_PUBKEY" ]]; then log_error "Reality 公钥不能为空"; fi
...
_load_existing_params() {
    ...
    LUODI_SECURITY=$(_ikv LUODI_SECURITY)
    LUODI_FLOW=$(_ikv LUODI_FLOW)
...
user_decision() {
    ...
    echo "" >&2
    echo -e "  ${CYAN}[1]${NC} 使用现有配置（推荐）— 仅刷新 IP + export.json 时间戳" >&2
    echo -e "  ${CYAN}[2]${NC} 重新读取代理参数 — 保留 WG 密钥，重新嗅探 Reality 参数" >&2
    echo -e "  ${CYAN}[3]${NC} 完全重置 — 重新生成 WG 密钥 + 重新嗅探（需重新运行 duijie.sh）" >&2
    echo -e "  ${CYAN}[4]${NC} 退出" >&2
    echo "" >&2

    local choice
    read -rp "  请选择 [1]: " choice </dev/tty || true
    choice="${choice:-1}"

    case "$choice" in
        1)  echo -e "${GREEN}[✓]${NC} 使用现有配置，仅刷新 IP 和 export.json..." >&2
            echo "reuse" ;;
        2)  echo -e "${GREEN}[✓]${NC} 重新读取代理参数（保留 WG 密钥）..." >&2
            echo "refresh" ;;
        3)  echo "full_reset" ;;
        4)  echo -e "${GREEN}[✓]${NC} 已退出" >&2; exit 0 ;;
        *)  echo -e "${YELLOW}[!]${NC} 无效选择，使用现有配置" >&2; echo "reuse" ;;
    esac
}
...
do_full_reset() {
    ...
    local confirm
    read -rp "  输入 'yes' 确认完全重置，其他取消: " confirm </dev/tty || true
...
prompt_relay_cleanup() {
    echo ""
    local yn
    read -rp "  是否连接中转机删除旧 WG Peer 数据？[y/N]: " yn </dev/tty || true
    [[ "${yn,,}" != "y" ]] && {
        echo ""
    ...
    echo ""
    local i
    read -rp "  中转机 IP [${relay_ip:-待输入}]: " i </dev/tty || true; [[ -n "$i" ]] && relay_ip="$i"
    [[ -z "$relay_ip" ]] && { log_warn "中转机 IP 为空，跳过远程清理"; return 0; }
    read -rp "  SSH 端口 [${relay_port}]: " i </dev/tty || true; relay_port="${i:-$relay_port}"
    read -rp "  SSH 用户 [${relay_user}]: " i </dev/tty || true; relay_user="${i:-$relay_user}"
...
" 2>/dev/null || echo '{}')
    ...
    read -rp "  确认执行？[y/N]: " yn </dev/tty || true
...
print_banner() {
    ...
    echo -e "${CYAN}║  构建日期: 2026-03-10  修复: 全选单显示 / TLS手动补全公钥   ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
}
